...
function startRound() {
    ...
    let optionsToShow = shuffledColors.slice(0, 4); // Take 3 random colors
...
answersButton.addEventListener('click', () => {
    ...
    feedback.innerHTML = `Réponses: <br>${answers}`; // Use innerHTML for <br> tags
...
function checkAnswer(guess) {
    if (guess === colors[currentKey]) {
        ...
        feedback.textContent = `Incorrect. La bonne réponse est ${colors[currentKey]}.`;
...
function endGame() {
    feedback.textContent = `Game Over! Score final: ${score}/10`;
